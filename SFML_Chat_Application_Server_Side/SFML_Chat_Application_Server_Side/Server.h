...
#define HEARTBEAT_INTERVAL 10000 //Milliseconds
...
struct ClientInfo
{
	...
	ClientInfo(const sf::IpAddress & l_ip, const PortNumber & l_port, const sf::Time & l_heartbeat):
		m_clientIP(l_ip), m_clientPORT(l_port), m_lastHeartbeat(l_heartbeat),
...
class Server;
using PacketHandler = std::function<void(sf::IpAddress&, const PortNumber &,
										const PacketID &, sf::Packet &, Server *)>;
using TimeourHandler = std::function<void(const ClientID &)>;
class Server
{
private:
	void Setup();
	...
	TimeourHandler m_timeoutHandler;
...
public:
	...
	Server(void(T::*l_handler)(sf::IpAddress&, const PortNumber &,
		const PacketID &, sf::Packet &, Server *), T * l_instance)
	{
	...
	bool Send(const sf::IpAddress & l_ip, const PortNumber & l_port, sf::Packet & l_packet);
